...
def findbodyguard(hmm:str):
    hmm.strip("\n")
    hmmlist = list(hmm)

    ahlist = []
    wtflist = []
    for i in range(len(hmmlist)-7):
        checklist = hmmlist[i:i+9]
        checked =[]
        for i in checklist:
            checked.append(check(ord(i)))
        if (checked == [0,1,1,1,0,1,1,1,0]):
            ahlist.append(checklist)
            wtflist.append(checklist[4])
    # exactly 3 bodyguards 라는 것은...
    # 3 대문자 옆에는 무조건 소문자여야 한다는 것입니다.
    
    print(ahlist)
    print("".join(wtflist))

findbodyguard('''kAewtloYgcFQaJNhHVGxXDiQmzjfcpYbzxlWrVcqsmUbCunkfxZWDZjUZMiGqhRRiUvGmYmvnJIHEmbT
